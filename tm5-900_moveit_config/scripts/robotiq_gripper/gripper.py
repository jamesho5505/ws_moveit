...
while counter < 1:
    counter = counter + 1
    # ActivationRequest ( clearand setrACT)
    ...
    time.sleep(0.01)
    # Read Gripperstatusuntil the activationiscompleted
    ser.write(b"\x09\x03\x07\xD0\x00\x01\x85\xCF")
    data_raw = ser.readline()
    print(data_raw)
    data = binascii.hexlify(data_raw)
    print("Response 2 ", data)
    # Response (if the activation ISNOT completed): 09 03 02 11 00 55 D5
    # Response (if the activation IScompleted): 09 03 02 31 00 4C 15
    time.sleep(1)
...
while(True):
    print("Close gripper")
    # Close the Gripperat full speed and full force
    ...
    time.sleep(2)
    print("Open gripper")
    # Openthe Gripperat full speed and full force
