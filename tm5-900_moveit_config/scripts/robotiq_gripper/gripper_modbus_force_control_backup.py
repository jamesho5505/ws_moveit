...
class RobotiqGripperNode(Node):
    def __init__(self):
        ...
        # --- PI 力控制器參數 ---
        self.declare_parameter('force_set_g', 1000.0)                 # 目標夾持力 (克)
        self.declare_parameter('rise_time_s', 0.25)                 # 目標力道達到設定值的時間 (秒)
        self.declare_parameter('deadband_g', 20.0)                  # 力控制死區 (克)，在此範圍內不進行調整
        self.declare_parameter('k_p', 0.0001)                         # 比例增益 (P gain)
        self.declare_parameter('k_i', 0.00000) 
        self.declare_parameter('k_d', 0.000000) 
        self.declare_parameter("dx_mm_max", 0.3)
        self.declare_parameter("cmd_rate_hz", 120.0)
        

        # 夾爪速度/力度(0-255)，保持中等速度與力(避免振盪)
        self.declare_parameter('rq_speed', 150)
        self.declare_parameter('rq_force', 200)

        # --- FSR 力感測器訊號濾波參數 ---
        self.declare_parameter('filter_alpha', 0.5)        # 指數濾波 α
        self.declare_parameter('median_k', 3)  
        self.declare_parameter("force_idx", 2)  
        
        # --- 混合控制相關
        self.declare_parameter('approach_target_mm', 5.0)   # 接觸前收合到的目標寬度
        ...
        self.declare_parameter('contact_backoff_mm', 0.5)    # 接觸後微退後
        ...
        # self.gripper.calibrate(0, 85) # 校準夾爪行程 (0mm - 85mm)
        saved_a = -0.3744493392070485
        saved_b = 86.12334801762114
        self.gripper._aCoef = saved_a
        self.gripper._bCoef = saved_b
        self.gripper.openmm = 85
        self.gripper.closemm = 0
        # self.gripper.goTomm(30, 255, 100)
        self.gripper.goTomm(20, 255, 100)
        self.close_mm, self.open_mm = 0.0, 85.0  # 夾爪最小/最大開度 (mm)
    ...
    def cb_force(self, msg: Float32MultiArray):
        """訂閱 /ESP32/force 的回呼函式，用來更新力道讀數"""
        if not msg.data:
            return
        i = int(self.get_parameter('force_idx').value)
        # 安全地從陣列中讀取力道值
        if -len(msg.data) <= i < len(msg.data):
            self.F_raw = max(0.0, float(msg.data[i]))  # 確保力道非負
    ...
    def get_target_force(self, t):
        """根據時間 t (秒) 設定力量追蹤曲線"""
        rise_time = self.get_parameter('rise_time_s').value
        target = self.get_parameter('force_set_g').value
        return target * (1 - math.exp(-t / rise_time))

    def pi_force_step(self, dt: float) -> float:
        if self.force_mode_start_time is None:
            t = 0.0
        else:
            t = time.time() - self.force_mode_start_time
        target = self.get_target_force(t)
        # target = self.get_parameter('force_set_g').value
        
        # deadband = float(self.get_parameter('deadband_g').value)
        deadband = target * 0.1 # 10% deadband
    ...
    def timer_cb(self):
        ...
        if self.mode == Mode.APPROACH:
            ...
            if self.detect_contact(dt):
                ...
                self.force_mode_start_time = time.time()  # 進入 FORCE 模式時開始計時
            ...
            if bool(self.get_parameter('hold').value):
                ...
                if abs(self.F_filt - target) < target * 0.1:
                    self.mode = Mode.HOLD
