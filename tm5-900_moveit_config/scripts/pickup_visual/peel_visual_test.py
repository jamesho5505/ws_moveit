...
VISION_WINDOW_NAME        = 'YOLO OBB - Visual Test'
PEELING_X_OFFSET = 13.0
PEELING_RX_OFFSET = -160.0
PEELING_Z_OFFSET =  95.0 #94.0 
...
class FuzzyKfController:
    ...
    #     return {
    #         'VS': np.array([self._trap(x, 0.001, 0.001, 0.002, 0.003) for x in universe]),
    #         'S' : np.array([self._tri (x, 0.001, 0.002, 0.003) for x in universe]),
    #         'M' : np.array([self._tri (x, 0.002, 0.003, 0.005) for x in universe]),
    #         'B' : np.array([self._tri (x, 0.004, 0.005, 0.006) for x in universe]),
    #         'VB': np.array([self._trap(x, 0.005, 0.006, 0.006, 0.006) for x in universe]),
    #     }
    def _precompute_output_mf(self):
        universe = self.KF_UNIVERSE
        return {
...
class HybridContourFollowingNode(Node):
    ...
    def __init__(
        ...
        self.contact_confirm_required = 0.10
        ...
        self.force_stable_required = 0.25
        ...
        self.force_integral_limit = 0.025  # 0.05

        self.force_deadband     = 0.15     # 0.3
        self.max_normal_step    = 0.12     # 0.2
    ...
    def ready_to_send_next_pvt(self):
        ...
        if elapsed_time < self.current_min_pvt_interval_sec():
            return False
        ...
        #     f'Hold PVT: phase={self._phase_name()}, '
        #     f'error={target_error_mm:.3f} mm, elapsed={elapsed_time:.3f}s'
        # )
        return False
    ...
    def current_min_pvt_interval_sec(self):
        ...
        if self.phase == self.PHASE_FORCE_REGULATION:
            return 0.20
        if self.phase == self.PHASE_FOLLOW_SURFACE:
            return 0.25
        return 0.25
    ...
    def send_pvt(self):
        ...
        # now = time.time()
        # if self.last_real_pvt_send_time is not None:
        #     pvt_send_interval_sec = now - self.last_real_pvt_send_time
        #     if pvt_send_interval_sec > 1e-6:
        #         self.get_logger().info(
        #             f"PVT rate = {1.0 / pvt_send_interval_sec:.2f} Hz"
        #         )
        ...
        script_sent = self.send_script(f"PVTPoint({pos},{vel},{pvt_time:.6f})")
        if script_sent:
