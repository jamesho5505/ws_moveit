...
if not local_ultralytics_init_file.exists():
    raise FileNotFoundError(f"找不到本地 ultralytics package: {local_ultralytics_init_file}")
...
class YOLOPickAndMove(Node):
    def __init__(self):
        ...
        # === Sampling ===
        self.NEEDED_SAMPLES = 10
        self.MAX_FRAMES = 300
        self.PATCH_K = 4

        # === tolerances ===
        self.POS_TOL_MM = 1.0
        self.ANG_TOL_DEG = 1.0
        self.CONSEC_OK = 1
    ...
    def slipped_at_lift(self, gobj_close):
        for _ in range(self.LIFT_CHECK_POLLS):
            time.sleep(self.LIFT_CHECK_DT)
    ...
    # ORIENTATION
    # =======================================================
    def compute_tcp_rpy_sxyz_from_yvec_and_plane_normal(self, y_vec_base, plane_normal_base):
        y_tcp = self._normalize(y_vec_base)
        n_plane = self._normalize(plane_normal_base)
        if y_tcp is None or n_plane is None:
            return None, None

        x_tcp = np.cross(n_plane, y_tcp)
        x_tcp = self._normalize(x_tcp)
        if x_tcp is None:
            return None, None

        z_tcp = np.cross(x_tcp, y_tcp)
        z_tcp = self._normalize(z_tcp)
        if z_tcp is None:
            return None, None

        if np.dot(z_tcp, np.array([0.0, 0.0, -1.0], dtype=float)) < 0.0:
            z_tcp = -z_tcp
    ...
    def capture_one_detection(self):
        ...
        top_valid = [i for i in top_all if i in valid]
        bot_valid = [i for i in bot_all if i in valid]

        top_sorted = sorted(top_valid, key=lambda ii: float(centers_px[ii, 0]), reverse=False)
        bot_sorted = sorted(bot_valid, key=lambda ii: float(centers_px[ii, 0]), reverse=False)

        sel_pos = 1  # chọn răng thứ 2 từ trái sang phải

        top_ok = len(top_sorted) > sel_pos
        bot_ok = len(bot_sorted) > sel_pos
        ...
        if top_ok and not bot_ok:
            region_name = "TOP"
            region_sorted = top_sorted
        elif bot_ok and not top_ok:
            region_name = "BOT"
            region_sorted = bot_sorted
        else:
            idx_sel_top = top_sorted[sel_pos]
            idx_sel_bot = bot_sorted[sel_pos]

            z_sel_top = Z_base[idx_sel_top]
            z_sel_bot = Z_base[idx_sel_bot]

            if z_sel_top > z_sel_bot:
                region_name = "TOP"
                region_sorted = top_sorted
            else:
                region_name = "BOT"
        ...
        ok_n, plane_n, dbg_n = self.estimate_tangent_normal_pca(
            depth_frame=depth,
            intr=intr,
            T_base_cam=T,
            u0=u0,
        ...
        if not ok_n or plane_n is None:
            # fallback plane normal
            plane_n = np.array([0.0, 0.0, 1.0], dtype=float)

        rpy_deg, M = self.compute_tcp_rpy_sxyz_from_yvec_and_plane_normal(y_vec, plane_n)
    ...
    def pickup_with_slip_retry(self, x_mm, y_mm, z_mm, rx, ry, rz,
        ...
        for attempt in range(1, self.MAX_REPICK_TRIES + 1):
            ...
            if st_close is not None and self.slipped_at_lift(st_close["gOBJ"]):
