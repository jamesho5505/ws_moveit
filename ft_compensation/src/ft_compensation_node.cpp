...
public:
    ...
    void run()
    {
        ...
        for (int row = 0; row < ROW && rclcpp::ok(); row += 3)
        {
            int pose_index = row / 3 + 1;
            RCLCPP_INFO(get_logger(),
                        "=== Pose %d/%d ===", pose_index, data_number_);
            RCLCPP_INFO(get_logger(),
                        "Move robot to desired pose, then press ENTER in this terminal.");
            {
                std::string dummy;
                std::getline(std::cin, dummy);
            }

            // 取當下的一筆作為「校正用」資料（維持原來行為）
